    ...
    def _build_model(self):
        ...
        with tf.variable_scope('embedding'):
           ...
           #                     dtype=tf.float32)
            ...
            #self.embedding_input = tf.expand_dims(self.embedding_input,1)
            #print( self.embedding_input)
        # define the GRU cell
        with tf.variable_scope('conv'):
            self.out1 = tf.keras.layers.Conv1D(64,3,padding='SAME',name='conv1')(self.embedding_input)
            self.out1 = tf.layers.average_pooling1d(self.out1,2,2,name='pool1') 
            self.out1 = tf.keras.layers.Conv1D(32,3,padding='SAME',name='conv2')(self.out1)
            self.out1 = tf.layers.average_pooling1d(self.out1,2,2,name='pool2') 
            self.out1 = tf.keras.layers.Conv1D(16,3,padding='SAME',name='conv3')(self.out1)
            self.out1 = tf.layers.average_pooling1d(self.out1,2,2,name='pool3') 
         
        self.out1 = tf.layers.flatten(self.out1,name='flatten')
        self.out1 = tf.nn.dropout(self.out1,self.dropout)    
        print(self.out1)    
...
#        print(self.state)            
       ...
       #     print self.state
        ...
        with tf.variable_scope('classifier'):
            self.w = tf.get_variable(name="W", shape=[2000,1],dtype=tf.float32)
   ...
   #     self.l2_loss = tf.nn.l2_loss(self.w,name="l2_loss")
          ...
          #  print self.w
          #  print self.b
        self.scores = tf.nn.xw_plus_b(self.out1,self.w,self.b,name="logits")
    ...
    def _train(self):
        ...
        with tf.Session() as sess:
            ...
            tf.train.write_graph(sess.graph_def, self.path, 'model_cnn.pbtxt') 
       ...
       #            print (y_batch_val) 
                    ...
                    print(" Epoch=",epoch," Batch=",batch," Training Loss: ",c, " Training Accuracy=",a)
                    c1,a1 = sess.run([self.loss,self.accuracy],feed_dict={self.X:X_batch_val,self.y:y_batch_val})
                    print(" Epoch=",epoch," Validation Loss: ",c1, " Validation Accuracy=", a1)
            ...
            self.saver.save(sess,self.path + 'model_ckpt_cnn')
            results = sess.run(self.prediction_probability,feed_dict={self.X:X_batch_val})
            print(results)
